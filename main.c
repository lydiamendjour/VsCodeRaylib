...
void drawList(int valueToSearch, int insertedElement, int insertPosition) {
    Node* current = head;
    ...
    while (current != NULL) {
        ...
        if (insertedElement != 0 && current->data == insertedElement) {
            circleColor = GREEN;
        } else {
            circleColor = (valueToSearch != 0 && current->data == valueToSearch) ? RED : BLACK;
        }

        DrawCircle(xOffset, yPos, radius, circleColor);
        DrawText(dataStr, xOffset - radius + 5, yPos - 10, 20, WHITE);
    ...
    // Highlight the position where the element was inserted
    if (insertedElement != 0) {
        int insertedXOffset = GetScreenWidth() / 2 + (3 * radius + 40) * (insertPosition - 1);
        DrawCircle(insertedXOffset, yPos, radius, GREEN);
        DrawText(TextFormat("%d", insertedElement), insertedXOffset - radius + 5, yPos - 10, 20, WHITE);
    }
}
void triselection() {
    if (head == NULL || head->next == NULL) {
        return; // The list is empty or has only one element, it is already sorted
    }
    ...
    for (current = head; current != NULL; current = current->next) {
        ...
        drawList(0); // Pass 0 to not highlight any element
        EndDrawing();

        // Introduce a delay to make the sorting visible
...
int main(void) {
    InitWindow(1200, 800, "Raylib Doubly Linked List ");
    ...
    while (!WindowShouldClose()) {
        ...
        if (insertingElement) {
            if (insertPosition == 0) {
                DrawText("Enter the position to insert the element:", 50, 160, 20, BLACK);
                ...
                DrawText("Enter the value of the element to insert:", 50, 230, 20, BLACK);
                ...
                if (IsKeyPressed(KEY_ENTER) && inputIndex > 0) {
                    ...
                    // Set the number of frames to highlight the inserted element
                    highlightFrames = 60; // Adjust the duration as needed
        ...
        if (state == 1) {
            DrawText("Enter the value of the node to delete:", 50, 150, 20, BLACK);
        ...
        if (state == 3) {
            DrawText("Enter the value to search:", 50, 150, 20, BLACK);
            DrawText(inputBuffer, 60, 180, 20, MAROON);
        ...
        if (highlightFrames > 0) {
            highlightFrames--;
            drawList(valueToSearch, insertValue, insertPosition);
        } else {
            drawList(valueToSearch, 0, 0);
        }  
        drawList(valueToSearch);

        DrawRectangle(50, 10, 100, 30, GRAY);
        DrawText("Empty", 65, 15, 20, WHITE);
        DrawRectangle(160, 10, 100, 30, GRAY);
        DrawText("Delete", 175, 15, 20, WHITE);
        DrawRectangle(270, 10, 100, 30, GRAY);
        DrawText("Search", 285, 15, 20, WHITE);
        DrawRectangle(380, 10, 100, 30, GRAY);
        DrawText("Insert", 395, 15, 20, WHITE);
        DrawRectangle(490, 10, 100, 30, GRAY);
